...
TcpAgent::TcpAgent(std::unique_ptr<EcnProcessor> ecn)
	: super(PT_TCP),
      ...
      ecn_processor_{ecn ? std::move(ecn) : std::make_unique<EcnProcessor>(this)}
{
...
//    delay_bind_init_one("overhead_");
    ...
    ecn_processor().delay_bind_init_all();
...
TcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
	...
	if (ecn_processor().delay_bind_dispatch(varName, localName, tracer)) return TCL_OK;
    // Wei
    if constexpr (!std::is_base_of_v<TracedVar, TracedInt>) {
...
double TcpAgent::rtt_timeout()
{
	...
	// clamping minrto_ before applying t_backoff_.
		if (t_rtxcur_ < minrto_ && !ecn_processor().use_rtt())
    ...
    if (timeout < 2.0 * tcp_tick_) {
		if (timeout < 0) {
			...
			fflush(stderr);
			exit(1);
		} else if (ecn_processor().use_rtt() && timeout < tcp_tick_)
			timeout = tcp_tick_;
		else
			timeout = 2.0 * tcp_tick_;
	}
	ecn_processor().stop_using_rtt_for_timeout();
	return (timeout);
}
...
void TcpAgent::output(int seqno, TcpXmissionReason reason)
{
	...
	ecn_processor().on_output(p, bool(is_retransmit));
	...
	if (seqno == 0) {
		...
		ecn_processor().on_output_tcp_syn(hf);
...
void TcpAgent::opencwnd()
{
	...
	if (cwnd_ < ssthresh_) {
		...
		if (ecn_processor().is_opencwnd_slowstart_adjustment_enabled())
			cwnd_ += ecn_processor().opencwnd_slowstart_multiplier() / cwnd_;
		else{
		...
		case 1:
			...
			if (ecn_processor().is_opencwnd_adjustment_enabled())
				increment = ecn_processor().opencwnd_multiplier() / cwnd_;
			else
...
TcpAgent::slowdown(TcpSlowdownFlags how)
{
	...
	double b = ecn_processor().get_dctcp_alpha();
	//fprintf(stderr,"%d\n",lldct_);
	//Wei: LLDCT
	if(lldct_)
	{
 ...
 */
void TcpAgent::EcnProcessor::ecn(int seqno) {
	if (seqno > agent().recover_ || 
			agent().last_cwnd_action_ == CongestionWindowAdjustmentReason::TIMEOUT) {
		agent().recover_ =  agent().maxseq_;
		agent().last_cwnd_action_ = CongestionWindowAdjustmentReason::ECN;
		if (agent().cwnd_ <= 1.0) {
			if (ecn_backoff_)
				agent().rtt_backoff();
			else {
...
auto TcpAgent::ecn_processor() -> EcnProcessor& {
	return *ecn_processor_;
}

auto EcnhatSenderCETracker::delay_bind_dispatch(
        const char *varName, const char *localName, TclObject *tracer) -> bool {
    if constexpr (!std::is_base_of_v<TracedVar, TcpAgent::TracedInt>) {
        if (processor_->agent().delay_bind_bool(varName, localName, "ecnhat_smooth_alpha_", &ecnhat_smooth_alpha_, tracer)) return true;
        if (processor_->agent().delay_bind(varName, localName, "ecnhat_alpha_", &ecnhat_alpha_ , tracer)) return true;
        if (processor_->agent().delay_bind(varName, localName, "ecnhat_g_", &ecnhat_g_ , tracer)) return true;
    }
    return false;
}

void EcnhatSenderCETracker::on_receive(Packet * pkt) {
    update_ecnhat_alpha(pkt);
}

void EcnhatSenderCETracker::delay_bind_init_all() {
    if constexpr (!std::is_base_of_v<TracedVar, TcpAgent::TracedInt>) {
        processor_->agent().delay_bind_init_one("ecnhat_smooth_alpha_");
        processor_->agent().delay_bind_init_one("ecnhat_alpha_");
        processor_->agent().delay_bind_init_one("ecnhat_g_");
    }
}
    
auto EcnhatSenderCETracker::get_dctcp_alpha() const -> double { 
    return ecnhat_alpha_; 
}

auto EcnhatSenderCETracker::is_opencwnd_adjustment_enabled() const -> bool { 
    return false; 
}

auto TcpAgent::EcnProcessor::should_slowdown_on_dup_ack() const -> bool {
	return get_send_tracker().should_slowdown_on_dup_ack();
}

void EcnhatSenderCETracker::on_timeout() {
    ecnhat_recalc_seq_ = processor_->agent().t_seqno_;
    ecnhat_maxseq_ = ecnhat_recalc_seq_;
}

void TcpAgent::EcnProcessor::on_timeout() {
    return get_send_tracker().on_timeout();
}

TcpAgent::EcnProcessor::EcnProcessor(TcpAgent * agent) 
    : ect_(false)
    , ecn_backoff_(0)
    ...
    , use_rtt_(false)
    , agent_{agent}
    , normal_send_{std::make_unique<NormalSenderCETracker>(agent)}
    , ecnhat_send_{std::make_unique<EcnhatSenderCETracker>(this)}
    , tcp_friendly_ecnhat_send_{std::make_unique<TcpFriendlyEcnhatSenderCETracker>(this)}
{
    if constexpr(std::is_base_of_v<TracedVar, TcpAgent::TracedInt>) {
        this->agent().bind("necnresponses_", &necnresponses_);
        // Mohammad
        this->agent().bind("ecnhat_", &ecnhat_);
        this->agent().bind("ecnhat_tcp_friendly_", &ecnhat_tcp_friendly_);
        this->agent().bind("ecnhat_enable_beta_", &ecnhat_enable_beta_);
        this->agent().bind("ecnhat_beta_", &ecnhat_beta_);
        this->agent().bind("ecnhat_quadratic_beta_", &ecnhat_quadratic_beta_);
	}
}

auto TcpAgent::EcnProcessor::is_enabled() const -> bool {
    return ecn_;
}

void TcpAgent::EcnProcessor::delay_bind_init_all() {
    ecn_ = 0;
    agent().delay_bind_init_one("ecn_");
    // Mohammad
    ecnhat_ = false;
    ecnhat_beta_ = 0.0;
    ...
    // Mohammad
    if constexpr (!std::is_base_of_v<TracedVar, TcpAgent::TracedInt>) {
        agent().delay_bind_init_one("ecnhat_");
        agent().delay_bind_init_one("ecnhat_beta_");
        agent().delay_bind_init_one("ecnhat_enable_beta_");
        agent().delay_bind_init_one("ecnhat_quadratic_beta_");
        agent().delay_bind_init_one("ecnhat_tcp_friendly_");
    }
    SetCWRonRetransmit_ = true;
    //  delay_bind_init_one("SetCWRonRetransmit_");
    old_ecn_ = 0;

    normal_send_->delay_bind_init_all();
    ecnhat_send_->delay_bind_init_all();
}


void TcpAgent::EcnProcessor::on_output(Packet * packet, bool is_retransmit) {
    if (ecn_) {
		hdr_flags::access(packet)->ect() = 1;  // ECN-capable transport
    }
    if (need_to_attach_cong_action(is_retransmit)) {
		attach_cong_action(hdr_flags::access(packet));
    }
}
    
void TcpAgent::EcnProcessor::attach_cong_action(hdr_flags * hf) {
	hf->cong_action() = TRUE;  // Congestion action.
	cong_action_ = false;
}
    
auto TcpAgent::EcnProcessor::need_to_attach_cong_action(bool is_retransmit) const -> bool {
	/*
	 ...
	 * - Sally Floyd, 6/5/08.
	 */
	return cong_action_ && (!is_retransmit || SetCWRonRetransmit_);
}

void TcpAgent::EcnProcessor::on_output_tcp_syn(hdr_flags * hf) {
    if (ecn_) {
        hf->ecnecho() = 1;
//      hf->cong_action() = 1;
        hf->ect() = 0;
...
auto TcpAgent::EcnProcessor::is_opencwnd_slowstart_adjustment_enabled() const -> bool {
    return ecnhat_enable_beta_;
}

auto TcpAgent::EcnProcessor::is_opencwnd_adjustment_enabled() const -> bool {
    return ecnhat_enable_beta_ || get_send_tracker().is_opencwnd_adjustment_enabled();
}

auto TcpAgent::EcnProcessor::opencwnd_slowstart_multiplier() const -> double {
    return ecnhat_beta_;
}

auto TcpAgent::EcnProcessor::opencwnd_multiplier() -> double {
    if (ecnhat_enable_beta_) {
        return ecnhat_beta_;
    } else {
		return get_send_tracker().opencwnd_multiplier();
    }
}

void TcpAgent::EcnProcessor::on_receive(Packet * pkt) {
    if (!ecn_) {
        return;
    }
    if (is_ecn_active(pkt))
        ecn(hdr_tcp::access(pkt)->seqno());
}


auto TcpAgent::EcnProcessor::can_reset_rto_backoff_tcp(Packet * pkt) const -> bool {
    return !ect_ || !ecn_backoff_ || !is_ecn_active(pkt);
}

void TcpAgent::EcnProcessor::reset_rto_backoff_tcp() { 
    ecn_backoff_ = 0; 
}

auto TcpAgent::EcnProcessor::can_open_cwnd(Packet * pkt) const -> bool {
    return !ect_ || !is_ecn_active(pkt) || (old_ecn_ && ecn_burst_);
}

void TcpAgent::EcnProcessor::on_receive_newack(Packet * pkt) {
	if (ect_) {
		if (!is_ecn_active(pkt))
			ecn_backoff_ = 0;
		update_ecn_burst(pkt);
	}
	if (!ect_ && is_ecn_active(pkt) && !hdr_flags::access(pkt)->cong_action())
		ect_ = true;

}

void TcpAgent::EcnProcessor::update_ecn_burst(Packet * pkt) {
    if (!ecn_burst_ && is_ecn_active(pkt))
        ecn_burst_ = true;
    else if (ecn_burst_ && !is_ecn_active(pkt))
        ecn_burst_ = false;
}

void TcpAgent::EcnProcessor::notify_sender_responded_to_congestion() {
	cong_action_ = true;
}

void TcpAgent::EcnProcessor::notify_sender_responded_to_ecn() {
	necnresponses_++;
}

auto TcpAgent::EcnProcessor::agent() -> TcpAgent& {
	return *agent_;
}

auto TcpAgent::EcnProcessor::agent() const -> TcpAgent const& {
	return *agent_;
}
    
auto TcpAgent::EcnProcessor::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer) -> bool {
    if (agent().delay_bind_bool(varName, localName, "ecn_", &ecn_, tracer)) return true;
    // Mohammad
    if constexpr (!std::is_base_of_v<TracedVar, TcpAgent::TracedInt>) {
        if (agent().delay_bind_bool(varName, localName, "ecnhat_", &ecnhat_, tracer)) return true;
        if (agent().delay_bind_bool(varName, localName, "ecnhat_enable_beta_", &ecnhat_enable_beta_ , tracer)) return true;
        if (agent().delay_bind(varName, localName, "ecnhat_beta_", &ecnhat_beta_ , tracer)) return true;
        if (agent().delay_bind_bool(varName, localName, "ecnhat_quadratic_beta_", &ecnhat_quadratic_beta_ , tracer)) return true;
        if (agent().delay_bind_bool(varName, localName, "ecnhat_tcp_friendly_", &ecnhat_tcp_friendly_, tracer)) return true;
        if (agent().delay_bind(varName, localName, "necnresponses_", &necnresponses_ , tracer)) return true;
    }
    if (agent().delay_bind_bool(varName, localName, "SetCWRonRetransmit_", &SetCWRonRetransmit_, tracer)) return true;
    if (agent().delay_bind_bool(varName, localName, "old_ecn_", &old_ecn_ , tracer)) return true;
    if (normal_send_->delay_bind_dispatch(varName, localName, tracer)) return true;
    if (ecnhat_send_->delay_bind_dispatch(varName, localName, tracer)) return true;
    return false;
}

auto TcpAgent::EcnProcessor::get_send_tracker() -> SenderCETracker& {
    return const_cast<SenderCETracker&>(
            const_cast<EcnProcessor const *>(this)->get_send_tracker());
}

auto TcpAgent::EcnProcessor::get_send_tracker() const -> SenderCETracker const& {
    if (ecnhat_) {
		if (ecnhat_tcp_friendly_) {
			return *tcp_friendly_ecnhat_send_;
		} else {
			return *ecnhat_send_;
		}
    } else {
...
auto TcpAgent::EcnProcessor::is_ecn_active(Packet * packet) const -> bool {
	return get_send_tracker().is_ecn_active(packet);
}
