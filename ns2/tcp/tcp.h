...
class TcpAgent : public BindCachingMixin<Agent> {
    ...
    friend class EcnhatSenderCETracker;
    friend class NormalSenderCETracker;
    friend class TcpFriendlyEcnhatSenderCETracker;
...
public:
	explicit TcpAgent(std::unique_ptr<EcnProcessor> ecn = nullptr);
...
private:
	std::unique_ptr<EcnProcessor> const ecn_processor_;
};
...
class TcpAgent::EcnProcessor {
    friend class EcnhatSenderCETracker;
    friend class TcpFriendlyEcnhatSenderCETracker;
public:
    explicit EcnProcessor(TcpAgent * agent);
    virtual ~EcnProcessor() = default;

    virtual void delay_bind_init_all();
    virtual auto delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer) -> bool;

    auto use_rtt() const -> bool { return use_rtt_; }
    void stop_using_rtt_for_timeout() { use_rtt_ = false; }
    // TODO: carefully check all the calls, the action might need to be put
    // into the EcnProcessor class
    auto is_enabled() const -> bool;
    auto get_dctcp_alpha() const -> double { 
        return get_send_tracker().get_dctcp_alpha();
    }

    void notify_sender_responded_to_congestion();
    void notify_sender_responded_to_ecn();
    auto has_sender_respondedn_to_congestion() const -> bool { return cong_action_; }

    auto should_slowdown_on_dup_ack() const -> bool;

    virtual auto can_open_cwnd(Packet * pkt) const -> bool;
    auto is_opencwnd_slowstart_adjustment_enabled() const -> bool;
    auto opencwnd_slowstart_multiplier() const -> double;
    auto is_opencwnd_adjustment_enabled() const -> bool;
    auto opencwnd_multiplier() -> double;

    auto can_reset_rto_backoff_tcp(Packet *pkt) const -> bool;
    void reset_rto_backoff_tcp();

    virtual void on_receive(Packet *pkt);
    virtual void on_receive_newack(Packet *pkt);
    void on_timeout();
    virtual void on_output(Packet *pkt, bool is_retransmit);
    void on_output_tcp_syn(hdr_flags *hf);

protected:
    auto ecn_burst() const -> bool { return ecn_burst_; }
    auto agent() -> TcpAgent&;
    auto agent() const -> TcpAgent const&;
    void update_ecn_burst(Packet * pkt);
    void ecn(int seqno);
    auto need_to_attach_cong_action(bool is_retransmit) const -> bool;
    void attach_cong_action(hdr_flags * hf);

    auto is_ecn_active(Packet * packet) const -> bool;

    virtual auto get_send_tracker() -> SenderCETracker&;
    virtual auto get_send_tracker() const -> SenderCETracker const&;
...
protected:
    ...
    int ecn_;
    bool ect_;         /* turn on ect bit now? */
    int ecnhat_;
    int ecn_backoff_; /* True when retransmit timer should begin
                                      to be backed off.  */
private:
    // needed for delay binding for TCL
    // TODO: ensure no other uses!!!!
    friend class FullTcpAgent;
    friend class DDTcpAgent;

    // TODO: probably beta is a completely different beast
    ...
    int dctcp_enable_ap;
    bool ecn_burst_;		/* True when the previous ACK packet
             *  carried ECN-Echo. */
    // TODO: maybe this does not belong here
    bool cong_action_;       /* Congestion Action.  True to indicate
                               that the sender responded to congestion. */
    bool use_rtt_;	     /* Use RTT for timeout for ECN-marked SYN-ACK */
    ...
    unique_ptr<SenderCETracker> const normal_send_;
    unique_ptr<SenderCETracker> const ecnhat_send_;
    unique_ptr<SenderCETracker> const tcp_friendly_ecnhat_send_;
};

class VegasTcpAgent::EcnProcessor : public TcpAgent::EcnProcessor {
    using TcpAgent::EcnProcessor::EcnProcessor;
    void on_receive(Packet *pkt) override;
};

class RenoTcpAgent::EcnProcessor : public TcpAgent::EcnProcessor {
    using TcpAgent::EcnProcessor::EcnProcessor;
    void on_receive(Packet *pkt) override;
};

class NewRenoTcpAgent::EcnProcessor : public TcpAgent::EcnProcessor {
    using TcpAgent::EcnProcessor::EcnProcessor;
    void on_receive(Packet *pkt) override;
};
