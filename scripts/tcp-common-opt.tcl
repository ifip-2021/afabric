...
TCP_pair instproc init {args} {
    ...
    $self instvar tcps tcpr apps
    global myAgent myApp
    eval $self next $args

    $self set tcps [new $myAgent]  ;# Sender TCP
    $self set tcpr [new $myAgent]  ;# Receiver TCP
    $self set apps [new $myApp]    ; # Sender App
...
TCP_pair instproc init_delay { } {
    ...
    if {$enable_delay} {
        ...
        if {$packet_properties_assigner_filename != {}} {
...
#For faster simulation.
    global ns delay_assigner_filename
    $self instvar apps tcps tcpr;# Sender TCP,  Receiver TCP
    $self instvar san dan  ;# memorize dumbell node (to attach)

    $self set san $snode
    $self set dan $dnode

    if {$delay_assigner_filename != {}} {
...
TCP_pair instproc start { flow_data } {
    global ns sim_end flow_gen
    $self instvar apps tcps tcpr id group_id
    ...
    $self set bytes [$flow_data get_total_size]
    ...
    if { $debug_mode == 1 } {
        puts "stats: [$ns now] start grp $group_id fid $id [$flow_data get_total_size] bytes"
    }
    ...
    $tcps set true_flow_size_ [$flow_data get_total_size]
    $tcpr set flow_remaining_ [$flow_data get_total_size]
    $apps send [$flow_data get_total_size]
}
...
#and each of them has its own flow id: init_fid + [0 .. nr-1]
    ...
    for {set i 0} {$i < $nr_pairs} {incr i} {
          ...
          if {[string compare $myApp "Application/Chunks"] == 0} {
              set app_seed [expr 17 * [lindex 0 $group_id] + 1244 * [lindex 1 $group_id] + $i]
              [$apair($i) set apps] set-seed $app_seed
          }
...
#flow length : pareto with mean $mean_nbytes bytes and shape parameter $shape.
    ...
    $rng1 seed $rands1
    $self set rv_flow_intval [create_exponential_rv $lambda]
...
#flow length : exp with mean mean_nbytes bytes.
    $self set_arrival_process\
        [create_exponential_rv $lambda] $rands1\
        [create_exponential_rv [expr 1.0/$mean_nbytes]] $rands2\
        [create_fixed_rv 1.0] 0\
        [create_fixed_rv 0] 0
}
...
#flow length: custom defined expirical cdf
    $self set_arrival_process\
        [create_exponential_rv $lambda] $rands1\
        [create_empirical_rv $cdffile 2] $rands2\
        [create_fixed_rv 1.0] 0\
        [create_fixed_rv 0] 0
}
...
#flow length: custom defined expirical cdf
    ...
    $self set rv_nbytes [create_empirical_rv $cdffile 2]
    $rv_nbytes use-rng $rng2
}
...
#fixed flow size
  $self set_arrival_process\
      [create_exponential_rv $lambda] $rand1\
      [create_fixed_rv $mean_nbytes] $rand2\
      [create_fixed_rv 1.0] 0\
        [create_fixed_rv 0] 0
}

Agent_Aggr_pair instproc set_rv { rv_name rv rv_seed } {
    $self instvar $rv_name
    set rng [new RNG]
    $rng seed $rv_seed
    $self set $rv_name $rv
    [set $rv_name] use-rng $rng
}

Agent_Aggr_pair instproc set_arrival_process {
        flow_intval_rv
        flow_intval_rng_seed 
        nbytes_rv
        nbytes_rng_seed
    } {
    
    $self set_rv "rv_flow_intval" $flow_intval_rv $flow_intval_rng_seed
    $self set_rv "rv_nbytes" $nbytes_rv $nbytes_rng_seed
}

Agent_Aggr_pair instproc set_PUarrival_process {lambda min_bytes max_bytes rand1 rand2} {
  $self set_arrival_process \
      [create_exponential_rv $lambda] $rand1\
      [create_uniform_tv $min_bytes $maxbytes] $rand2\
      [create_fixed_rv 1.0] 0\
        [create_fixed_rv 0] 0
}
...
# according to the flow arrival process.
 ...
 #   $self instvar fid
    ...
    set flow_size_pkts [expr ceil ([$rv_nbytes value])]
    set flow_size_bytes [expr $flow_size_pkts * 1460]

    $ns at $tnext "$apair($pid) start [new FlowData $flow_size_bytes]"
...
Agent_Aggr_pair instproc check_if_behind {} {
    global ns
    global flow_gen sim_end myApp init_fid
    $self instvar apair
    ...
    set t [$ns now]
    # The test for $tnext can only pass if $tnext has not changed
    if { $flow_gen < $sim_end && $tnext < [expr $t + 0.0000002] } { #create new flow
        ...
        if {[string compare $myApp "Application/Chunks"] == 0} {
            set app_seed [expr 17 * [lindex 0 $group_id] + 1244 * [lindex 1 $group_id] + $nr_pairs]
            [$apair($nr_pairs) set apps] set-seed $app_seed
        }
...
#
    ...
    if { [info exists logfile] } {
        ...
        puts $logfile "$tmp_pkts $fldur $rttimes $group_id $pid $deadline $early_terminated"
        flush $logfile
    }
...
proc finish {} {
    global ns flowlog droplog drop_sink
    global sim_start
    global enableNAM namfile

    $drop_sink print-stats $droplog
    close $droplog
    $ns flush-trace
    close $flowlog
