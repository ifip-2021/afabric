...
set goodQueueSize [next_arg]
set connections_per_pair [next_arg]
set nbytes_rv_cmd [next_arg]
set flow_intval_rv_cmd [next_arg]
set chunks_alpha [next_arg]
set use_alpha_probability [next_arg]
set max_num_chunks [next_arg]
...
set eof_min_rto [next_arg]
set no_eof_min_rto [next_arg]
...
set use_true_remaining_size [next_arg]
set rtx_on_eof [next_arg]
set reset_window_on_eof [next_arg]
set enable_afabric_ecn [next_arg]
...
Agent/TCP/FullTcp set use_true_remaining_size_ $use_true_remaining_size
Agent/TCP/FullTcp set eof_minrto_ $eof_min_rto
Agent/TCP/FullTcp set no_eof_minrto_ $no_eof_min_rto
...
Agent/TCP/FullTcp set rtx_on_eof_ $rtx_on_eof
Agent/TCP/FullTcp set reset_window_on_eof_ $reset_window_on_eof
Agent/TCP/FullTcp set nodelay_ true; # disable Nagle
Agent/TCP/FullTcp set segsperack_ $ackRatio;
Agent/TCP/FullTcp set interval_ 0.000006
Agent/TCP/FullTcp set afabric_ecn_enabled_ $enable_afabric_ecn
...
if {[string compare $sourceAlg "DCTCP-Sack"] == 0} {
    set myAgent "Agent/TCP/FullTcp/Sack$tcpSuffix"
    ...
    set myAgent "Agent/TCP/FullTcp/Sack/DDTCP$tcpSuffix";
    Agent/TCP set ecnhat_ true
    Agent/TCPSink set ecnhat_ true
    Agent/TCP set ecnhat_g_ $DCTCP_g
    Agent/TCP set lldct_ false
} elseif {[string compare $sourceAlg "MinTCP-Sack"] == 0} {
    set myAgent "Agent/TCP/FullTcp/Sack/MinTCP$tcpSuffix"
    Agent/TCP set ecnhat_ true
    Agent/TCPSink set ecnhat_ true
    Agent/TCP set ecnhat_g_ $DCTCP_g
    Agent/TCP set lldct_ false
...
if {$chunks_alpha != {}} {
    set myApp "Application/Chunks"
    Application/Chunks set packet_size_bytes_ $pktSize
    Application/Chunks set header_size_bytes_ 40
    Application/Chunks set alpha_ $chunks_alpha
    Application/Chunks set link_rate_ $link_rate
    Application/Chunks set max_num_chunks_ $max_num_chunks
    if {$use_alpha_probability != {}} {
        Application/Chunks set use_alpha_probability_ $use_alpha_probability
    } else {
        Application/Chunks set use_alpha_probability_ -1.0
    }
} else {
    set myApp "Application/PIAS"
}
...
Queue/DropTail set good_limit_ $goodQueueSize
...
Queue/DropTail set drop_low_prio_ $drop_low_prio_
...
for {set j 0} {$j < $S } {incr j} {
    for {set i 0} {$i < $S } {incr i} {
        if {$i != $j} {
                ...
                $agtagr($i,$j) set_arrival_process\
                    [eval $flow_intval_rv_cmd]\
                    [expr 17*$i+1244*$j]\
                    [eval $nbytes_rv_cmd]\
                    [expr 33*$i+4369*$j]\
